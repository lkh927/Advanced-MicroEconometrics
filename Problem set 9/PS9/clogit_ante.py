...
def q(theta, y, x): 
    ...
    Returns
        (N,) vector. 
    '''
    return - np.sum(loglikelihood(theta, y, x))
...
def loglikelihood(theta, y, x): 
    ...
    # denominator 
    denom = np.sum(np.exp(v), axis=1) # No keepdims=True here, because we want the denom to be 1-dimensional
...
def choice_prob(theta, x):
    ...
    # denominator 
    denom = np.sum(np.exp(v), axis=1, keepdims=True) #keepdims=True keeps the second dimension (N,1)
    assert denom.ndim == 2, f'denom should have ndim == 2, got {denom.ndim}' 
    # denom must be (N,1) so we can divide an (N,J) matrix with it without broadcasting errors
